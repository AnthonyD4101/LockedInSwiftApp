...
class TaskViewModel: ObservableObject {
    ...
    func tasksFor(date: Date) -> [UserTask] {
    ...
    func addTask(title: String, description: String, dueDate: Date, subtasks: [Subtask]) {
        let newTask = UserTask(name: title, description: description, date: dueDate, subtasks: subtasks)
    ...
    func addSubtask(to task: UserTask, name: String) {
        if let index = tasks.firstIndex(where: { $0.id == task.id }) {
            tasks[index].subtasks.append(Subtask(name: name))
        }
    }
    
    func removeSubtask(from task: UserTask, at index: Int) {
